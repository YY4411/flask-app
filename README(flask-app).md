...
@app.route("/")
def hello():
    return "<h1>Welcome to Furkan's Web Page</h1>"
...
```bash
docker build -t "yildiray4411/flask-app1:1.0" .
docker image ls
```
...
```bash
docker run -d --name welcome -p 80:80 yildiray4411/flask-app1:1.0
docker ps
```
...
```bash
docker push yildiray4411/flask-app1
```
